<template>
  <AdminLayout title="Editar Agenda">
    <div class="card card-warning">
      <div class="card-header">
        <h3 class="card-title"><i class="fas fa-edit"></i> Editar Agenda</h3>
      </div>

      <form @submit.prevent="actualizar">
        <div class="card-body">
          <div class="form-group">
            <label>Cliente</label>
            <select v-model="form.cliente_id" class="form-control">
              <option v-for="c in clientes" :key="c.id" :value="c.id">{{ c.nombre }}</option>
            </select>
          </div>

          <div class="form-group">
            <label>Empleado</label>
            <select v-model="form.empleado_id" class="form-control">
              <option v-for="e in empleados" :key="e.id" :value="e.id">{{ e.nombre }}</option>
            </select>
          </div>

          <div class="form-group">
            <label>Servicio</label>
            <select v-model="form.servicio_id" class="form-control">
              <option v-for="s in servicios" :key="s.id" :value="s.id">{{ s.nombre }}</option>
            </select>
          </div>

          <div class="form-group">
            <label>Fecha y Hora</label>
            <input type="datetime-local" v-model="form.fecha_hora" class="form-control">
          </div>

          <div class="form-group">
            <label>Estado</label>
            <select v-model="form.estado" class="form-control">
              <option value="pendiente">Pendiente</option>
              <option value="confirmada">Confirmada</option>
              <option value="cancelada">Cancelada</option>
              <option value="completada">Completada</option>
            </select>
          </div>
        </div>

        <div class="card-footer text-right">
          <Link href="/agendas" class="btn btn-secondary">Cancelar</Link>
          <button type="submit" class="btn btn-warning">Actualizar</button>
        </div>
      </form>
    </div>
  </AdminLayout>
</template>

<script setup>
import { Link, useForm } from '@inertiajs/vue3'
import AdminLayout from '@/Layouts/AdminLayout.vue'

// Aquí definimos props correctamente
const props = defineProps({
  agenda: Object,
  clientes: Array,
  empleados: Array,
  servicios: Array
})

// Inicializamos el form usando los datos de la agenda
const form = useForm({
  cliente_id: String(props.agenda.cliente_id || ''),
  empleado_id: String(props.agenda.empleado_id || ''),
  servicio_id: String(props.agenda.servicio_id || ''),
  fecha_hora: props.agenda.fecha_hora ? props.agenda.fecha_hora.substring(0,16) : '',
  estado: props.agenda.estado || 'pendiente'
})

// Función para actualizar
const actualizar = () => {
  form.put(`/agendas/${props.agenda.id}`)
}
</script>
